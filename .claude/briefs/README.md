...
Pathbuilder JSON export format (informally documented; structure has been stable for years). Key fields:

- `build.name` — character name
- `build.class` — class name
- `build.level` — character level
- `build.ancestry`, `build.heritage`, `build.background` — core identity
- `build.abilities` — ability scores (raw, not modifiers)
- `build.attributes` — ancestry HP, class HP, speed
- `build.proficiencies` — skill and save ranks (numeric: 0=untrained, 2=trained, 4=expert, etc.)
- `build.feats` — nested list of feats with source metadata
- `build.specials` — class features
- `build.weapons`, `build.armor` — equipment
- `build.lores` — lore proficiencies
...
Character JSON files are committed to the public repo. They contain no PII beyond what's already on a character sheet (character name, class, level, stats). This is acceptable for the project owner's own party. When Phase B ships for public users, uploaded characters will live in a database (not the repo) with appropriate privacy controls.
