...
Strict PF2e damage resolution order. Key function: `resolve_strike()`.
...
`RoundState` — mutable state for turn evaluation. Shallow-clone with frozenset conditions for cheap branching.

### sim/search.py (will be added CP5.1 3b)
- `SearchConfig` dataclass
- `beam_search_turn()` — per-character beam search
- `adversarial_enemy_turn()` — enemy sub-search with sign-flipped scoring
- `simulate_round()` — full-round evaluation

### sim/initiative.py (will be added CP5.1 3b)
`roll_initiative()` — seeded Perception + d20 roll, sorted with tiebreakers.
...
When Phase B (Pathbuilder importer) ships, uploaded characters land in this directory.
